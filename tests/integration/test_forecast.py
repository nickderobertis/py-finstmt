...
def adjust_forecast_methods(
    stmts: FinancialStatements, adjust_dict: Dict[str, Sequence[str]]
):
...
class TestForecastStockrowCAT(ForecastTest):
    ...
    def test_annual_no_balance(self, annual_stockrow_stmts_cat: FinancialStatements):
        super().test_annual(
            annual_stockrow_stmts_cat,
            data=FCST_STOCKROW_CAT_NO_BALANCE_A_INDEX_DATA_DICT,
            balance=False,
            name="fcst_stockrow_cat_no_balance",
            ignore_keys=["gross_ppe", "dep"],
        )

    def test_annual_change_bs_diff(
    ...
    def test_quarterly(self, quarterly_stockrow_stmts_cat: FinancialStatements):
        super().test_quarterly(
            quarterly_stockrow_stmts_cat, ignore_keys=["gross_ppe", "dep"]
        )
